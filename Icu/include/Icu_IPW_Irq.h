...
#define ICU_IPW_IRQ_AR_RELEASE_MINOR_VERSION        0
#define ICU_IPW_IRQ_AR_RELEASE_REVISION_VERSION     3
#define ICU_IPW_IRQ_SW_MAJOR_VERSION                1
#define ICU_IPW_IRQ_SW_MINOR_VERSION                1
...
#define Icu_IPW_ReportEvents(hwChannel, overflow) \
        do \
        { \
            Icu_ReportEvents((hwChannel), (overflow));\
        } while(0)
/**
* @brief Mapping macro between get edge numbers eTimer functionality and high level function
*
...
#define Icu_IPW_ReportWakeupAndOverflow(hwChannel, overflow) \
        do \
        { \
            Icu_ReportWakeupAndOverflow((hwChannel), (overflow));\
        } while(0)
 ...
 */
 #define Icu_IPW_Timestamp(hwChannel,capturedWords,bufferPtr,overflow) \
        do \
        { \
            Icu_Timestamp((hwChannel),(capturedWords),(bufferPtr),(overflow));\
        } while(0)
...
*/
 #define Icu_IPW_SignalMeasurement(hwChannel, activePulseWidth, period, overflow) \
        do \
        { \
            Icu_SignalMeasurement((hwChannel), (activePulseWidth), (period), (overflow));\
        } while(0)
#endif
