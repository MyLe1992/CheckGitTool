...
#define ICU_SW_MINOR_VERSION_C              1
...
*           defined at block scope 
*/
volatile STATIC VAR(Icu_ValueType, ICU_VAR) Icu_aPeriod[ICU_MAX_CHANNEL];
...
*           defined at block scope 
*/
volatile STATIC VAR(Icu_ValueType, ICU_VAR) Icu_aActivePulseWidth[ICU_MAX_CHANNEL];
...
#define ICU_START_SEC_VAR_NO_INIT_32_NO_CACHEABLE
/*
 * @violates @ref Icu_c_REF_1 only preprocessor statements and
 *        comments before "#include".
 */
...
#define ICU_STOP_SEC_VAR_NO_INIT_32_NO_CACHEABLE
/*
 * @violates @ref Icu_c_REF_1 only preprocessor statements and
 *        comments before "#include".
 */
 ...
 */
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidateCallSetMode(Icu_ModeType Mode)
{
    ...
    if (Mode != Icu_CurrentMode)
    {
        ...
        for (numChl = 0U; numChl < Icu_pCfgPtr->Icu_MaxChannels; numChl++)
                                                                      ...
                                                                      )
{
    ...
    else
    {
        ...
        if ((Channel > Icu_pCfgPtr->Icu_MaxChannels) ||\
                                                    ...
                                                    )
{
    ...
    if (((boolean)TRUE == Icu_IPW_Get_Overflow((*(Icu_pCfgPtr->Icu_ChannelId))[Channel])) || \
 ...
 * @retval      E_NOT_OK The function call is invalid
 *
 * @implements  Icu_ValidatePtrInit_Activity
 *
 * */
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidatePtrInit(P2CONST(Icu_ConfigType, AUTOMATIC, ICU_APPL_CONST) ConfigPtr)
{
    ...
    if (NULL_PTR == ConfigPtr)
    {
        ...
        Det_ReportError((uint16)ICU_MODULE_ID, 0U, ICU_INIT_ID, ICU_E_PARAM_CONFIG);
        valid = (Std_ReturnType)E_NOT_OK;
    }
    return valid;
}
...
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidateActivation(VAR(Icu_ActivationType, AUTOMATIC) Activation)
{
    ...
    {
        Det_ReportError((uint16)ICU_MODULE_ID, 0U, ICU_SETACTIVATIONCONDITION_ID,\
...
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidateTimestamp(VAR(Icu_ChannelType, AUTOMATIC) Channel,
                                                                  P2CONST(Icu_ValueType, AUTOMATIC, ICU_APPL_DATA) pBufferPtr,
                                                                 ...
                                                                 )
{
    ...
    {
        Det_ReportError((uint16)ICU_MODULE_ID, 0U, ICU_STARTTIMESTAMP_ID, ICU_E_PARAM_BUFFER_PTR);
    }
    ...
    {
        Det_ReportError((uint16)ICU_MODULE_ID, 0U, ICU_STARTTIMESTAMP_ID,\
 ...
 *
 * */
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidatePtrDutyCycle(P2CONST(Icu_DutyCycleType, AUTOMATIC, ICU_APPL_DATA) DutyCycleValues)
{
    VAR(Std_ReturnType, AUTOMATIC) valid = (Std_ReturnType)E_OK;
    
    if (NULL_PTR == DutyCycleValues)
    {
        Det_ReportError((uint16)ICU_MODULE_ID, 0U, ICU_GETDUTYCYCLEVALUES_ID, \
 ...
 * @retval      E_NOT_OK The function call is invalid
 *
 * @implements Icu_ValidateVersionInfoCall_Activity
 *
 * */
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidateVersionInfoCall(P2CONST(Std_VersionInfoType, AUTOMATIC, ICU_APPL_DATA) versioninfo)
{
    VAR(Std_ReturnType, AUTOMATIC) valid = (Std_ReturnType)E_OK;
 ...
 * @retval      E_NOT_OK The function call is invalid
 *
 * @implements  Icu_ValidateClockModeCall_Activity
 *
 * */
...
LOCAL_INLINE FUNC(Std_ReturnType, ICU_CODE) Icu_ValidateGetInputLevel(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    if ((ICU_VALID_CHANNEL_INPUT_LEVEL <= (*(Icu_pCfgPtr->Icu_ChannelId))[Channel]))
    {
...
FUNC(void, ICU_CODE) Icu_Init(P2CONST(Icu_ConfigType, AUTOMATIC, ICU_APPL_CONST) ConfigPtr)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
...
#endif
            Icu_IPW_InitGlobalConfiguration((Icu_IPW_GlobalConfigurationType*) \
                                            Icu_pCfgPtr->Icu_GlobalConfiguration, (boolean)TRUE
                                           );

            for (numChl = 0U; numChl < Icu_pCfgPtr->Icu_MaxChannels; numChl++)
            {
                ChannelConfig = &(*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[numChl];
                Icu_IPW_InitChannel((*(Icu_pCfgPtr->Icu_ChannelId))[numChl],
                                    ChannelConfig->Icu_ParamValue,
                                    ChannelConfig->Icu_Channel_Mode
#ifdef ICU_EXTENDED_FILTERS_AVAILABLE
                                    ,ChannelConfig->Icu_RisingEdge_Filter,
                                    ChannelConfig->Icu_FallingEdge_Filter
#endif /* ICU_EXTENDED_FILTERS_AVAILABLE */
                                   );
            ...
            Icu_IPW_InitDmaConfiguration(Icu_pCfgPtr->Icu_DmaConfig->u8NoDmaChConfigured, \
                                         Icu_pCfgPtr->Icu_DmaConfig->DmaReqConfig
                                        );
#endif
...
FUNC(void, ICU_CODE) Icu_DeInit(void)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        for (numChl = 0U; numChl < Icu_pCfgPtr->Icu_MaxChannels; numChl++)
        {
            Icu_IPW_DeInitChannel((*(Icu_pCfgPtr->Icu_ChannelId))[numChl]);
            /* Clear channel internal state */
            Icu_aChannelState[numChl] =  0x00U;
        }
        Icu_IPW_InitGlobalConfiguration((Icu_IPW_GlobalConfigurationType*) \
                                        Icu_pCfgPtr->Icu_GlobalConfiguration, (boolean)FALSE
                                       );
#if ((ICU_SIGNALMEASUREMENT_USES_DMA == STD_ON) || (ICU_TIMESTAMP_USES_DMA == STD_ON))
        Icu_IPW_DeInitDmaConfiguration(Icu_pCfgPtr->Icu_DmaConfig->u8NoDmaChConfigured, \
                                       Icu_pCfgPtr->Icu_DmaConfig->DmaReqConfig
                                      );
#endif
                ...
                if (Icu_CurrentMode != Mode)
                {
                    ...
                    for (numChl = 0U; numChl < Icu_pCfgPtr->Icu_MaxChannels; numChl++)
                    {
                        ...
                        if (ICU_MODE_SLEEP == Mode)
                        {
                            ...
                            {
                                Icu_IPW_SetSleepMode((*(Icu_pCfgPtr->Icu_ChannelId))[numChl]);
                            }
                        }
                        else
                        {
                            ...
                            {
                                Icu_IPW_SetNormalMode((*(Icu_pCfgPtr->Icu_ChannelId))[numChl]);
                            }
                        }
                    }
                                               ...
                                               )
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        if ((Std_ReturnType)E_OK == Icu_ValidateActivation(Activation))
        {
            ...
            Icu_IPW_SetActivationCondition((*(Icu_pCfgPtr->Icu_ChannelId))[Channel],
                                           Activation,
                                           (*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Icu_Channel_Mode
                                          );
#if ( ICU_VALIDATE_PARAMS == STD_ON)
...
FUNC(Icu_InputStateType, ICU_CODE) Icu_GetInputState(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        {
            tempState = Icu_IPW_GetInputState((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
        }
        else
        {
            /* does nothing*/
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
               ...
               )
            {
                ...
                if(NoMclDmaChannel != (*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel)
                {
                    ...
                    tcd_config[Channel].saddr = Icu_IPW_GetStartAddress \
                                                        ((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
                    /* @violates @ref Icu_c_REF_7 Violates MISRA 2004 Rule 11.1, Cast from unsigned long
                     * to pointer.*/
                    /* Compiler_Warning: This warning is thrown because of a conversion between a Icu_ValyeType pointer and a uint32 */
                    tcd_config[Channel].daddr = (uint32)BufferPtr;
                    tcd_config[Channel].ssize = (uint32)ICU_DMA_SIZE;
                    tcd_config[Channel].dsize = (uint32)ICU_DMA_SIZE;
                    tcd_config[Channel].soff  = (uint32)0;
                    tcd_config[Channel].doff  = (uint32)ICU_DMA_OFFSET;
                    tcd_config[Channel].smod  = (uint32)0;
                    tcd_config[Channel].dmod  = (uint32)0;
                    if ((NotifyInterval >= BufferSize) || (NotifyInterval == (uint16)0))
                    {
                        tcd_config[Channel].num_bytes = ICU_DMA_NUM_BYTES;
                        tcd_config[Channel].iter = BufferSize;
                    }
                    else if (NotifyInterval < BufferSize)
                    {
                        tcd_config[Channel].num_bytes = ICU_DMA_NUM_BYTES;
                        tcd_config[Channel].iter = NotifyInterval;
                    }
                    else
                    {
                        /* does nothing*/
...
#endif
                Icu_IPW_StartTimestamp((*(Icu_pCfgPtr->Icu_ChannelId))[Channel], bDmaSupport);
#if ( ICU_VALIDATE_PARAMS == STD_ON)
            }
#endif
#if ( ICU_VALIDATE_CALL_AND_CHANNEL == STD_ON)
...
#endif
            Icu_IPW_StopTimestamp((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
...
FUNC(void, ICU_CODE) Icu_ResetEdgeCount(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        Icu_IPW_ResetEdgeCount((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
...
FUNC(void, ICU_CODE) Icu_EnableEdgeCount(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    if((Std_ReturnType)E_OK == Icu_ValidateModeForSetUpFeature(ICU_ENABLEEDGECOUNT_ID))
    {
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
            ...
            /* Clear overflow flag */
            Icu_SetBitChState(Channel, ICU_CHANNEL_STATE_RUNNING);
            Icu_IPW_EnableEdgeCount((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
#if (ICU_VALIDATE_CALL_AND_CHANNEL == STD_ON)
        }
    Icu_EndValidateCallAndChannel(valid);
#endif
...
FUNC(void, ICU_CODE) Icu_DisableEdgeCount(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    {
#endif
        Icu_IPW_DisableEdgeCount((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
        Icu_ClearBitChState(Channel, ICU_CHANNEL_STATE_RUNNING);
#if (ICU_VALIDATE_CALL_AND_CHANNEL == STD_ON)
    }
    Icu_EndValidateCallAndChannel(valid);
...
#endif
            edgeNumbers = \
                Icu_IPW_GetEdgeNumbers((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
#if (ICU_VALIDATE_PARAMS == STD_ON)
#if (ICU_OVERFLOW_NOTIFICATION_API == STD_OFF)
        }
#endif
...
#endif
            Icu_SetBitChState(Channel, ICU_CHANNEL_STATE_RUNNING);
            Icu_IPW_EnableEdgeDetection((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
#if (ICU_VALIDATE_CALL_AND_CHANNEL == STD_ON)
        }
        Icu_EndValidateCallAndChannel(valid);
#endif
...
FUNC(void, ICU_CODE) Icu_DisableEdgeDetection(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    {
#endif
        Icu_IPW_DisableEdgeDetection((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
        Icu_ClearBitChState(Channel, ICU_CHANNEL_STATE_RUNNING);
#if (ICU_VALIDATE_CALL_AND_CHANNEL == STD_ON)
    }
    Icu_EndValidateCallAndChannel(valid);
        ...
        if ((Std_ReturnType)E_OK == valid)
        {
            ...
            Icu_ClearBitChState(Channel, ICU_CHANNEL_STATE_OVERFLOW);
                ...
                if(NoMclDmaChannel != (*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[Channel].Mcl_DmaChannel)
                {
                     ...
                     * to pointer.*/
                    /* Compiler_Warning: This warning is thrown because of a conversion between a Icu_ValyeType pointer and a uint32 */
                    tcd_config[Channel].daddr = (uint32)&Icu_aDmaBuffer[Channel][0];
                    tcd_config[Channel].ssize = (uint32)ICU_DMA_SIZE;
                    tcd_config[Channel].dsize = (uint32)ICU_DMA_SIZE;
                    tcd_config[Channel].soff  = (uint32)0;
                    tcd_config[Channel].doff  = (uint32)ICU_DMA_OFFSET;
                    tcd_config[Channel].smod  = (uint32)0;
                    tcd_config[Channel].dmod  = (uint32)ICU_DMA_MAJORLOOP_COUNT;
                    tcd_config[Channel].num_bytes = ICU_DMA_NUM_BYTES;
                    tcd_config[Channel].iter = ICU_DMA_MAJORLOOP_COUNT;
            ...
            Icu_IPW_StartSignalMeasurement((*(Icu_pCfgPtr->Icu_ChannelId))[Channel],
...
FUNC(void, ICU_CODE) Icu_StopSignalMeasurement(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    {
#endif
        Icu_IPW_StopSignalMeasurement((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
        Icu_ClearBitChState(Channel, ICU_CHANNEL_STATE_RUNNING);
        ...
        if ((boolean)FALSE == Icu_GetOverflow(Channel, ICU_GETTIMEELAPSED_ID, ICU_E_MEASUREMENT_OVERFLOW))
        {
            ...
            if ((Std_ReturnType)E_OK == Icu_ValidateSignalMeasurementProperty
                    (Channel,\
                     ...
                     (uint16)ICU_PERIOD_TIME | (uint16)ICU_ACTIVE_TIME,
...
FUNC (void, ICU_CODE) Icu_SetClockMode(VAR(Icu_SelectPrescalerType, AUTOMATIC) Prescaler)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        {
#endif
            Icu_IPW_SelectGlobalPrescaler((Icu_IPW_GlobalConfigurationType*) \
                                          Icu_pCfgPtr->Icu_GlobalConfiguration, Prescaler
                                         );

            for (numChl = 0U; numChl < Icu_pCfgPtr->Icu_MaxChannels; numChl++)
            {
                ChannelConfig = &(*(Icu_pCfgPtr->Icu_ChannelConfigPtr))[numChl];
                Icu_IPW_SelectPrescaler((*(Icu_pCfgPtr->Icu_ChannelId))[numChl],
                                        ChannelConfig->Icu_ParamValue, Prescaler
                                       );
            }
#if (ICU_VALIDATE_PARAMS == STD_ON)
        }
...
FUNC(Icu_LevelType, ICU_CODE) Icu_GetInputLevel(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    if ((Std_ReturnType)E_OK == valid)
    {
        ...
        {
#endif
            level = (Icu_LevelType)Icu_IPW_GetInputLevel((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
#if (ICU_VALIDATE_CALL_AND_CHANNEL == STD_ON)
        }
    }
    Icu_EndValidateCallAndChannel(valid);
...
FUNC(void,ICU_CODE) Icu_SignalMeasurementDmaProcessing(VAR(Icu_ChannelType, AUTOMATIC)Channel)
{
    ...
    VAR(Icu_ActivationType, AUTOMATIC) Activation = \
                       (Icu_ActivationType)((Icu_pTempPtr->Icu_ParamValue) >> ICU_EDGE_PARAM_SHIFT);
    VAR(Icu_ValueType, AUTOMATIC) BufferValue1 = Icu_aDmaBuffer[Channel][1];
    VAR(Icu_ValueType, AUTOMATIC) BufferValue2 = Icu_aDmaBuffer[Channel][2];
    VAR(Icu_ValueType, AUTOMATIC) BufferValue3 = Icu_aDmaBuffer[Channel][3];
    /* @violates @ref Icu_c_REF_7 Violates MISRA 2004 Rule 11.1, Cast from unsigned long
    * to pointer.*/
    /* Compiler_Warning : This warning is thrown because of a conversion between a Icu_ValyeType pointer and a uint32 */
    ...
    if(ICU_RISING_EDGE == Activation)
    {
        ...
        {
            Icu_aActivePulseWidth[Channel] = \
                        (ICU_CAPTURE_REGISTER_MASK - BufferValue1) + BufferValue2;
        }
    }
    else
    {
        if(BufferValue3 > BufferValue2)
        {
            /* Ignore the first data */
            Icu_aActivePulseWidth[Channel] = BufferValue3 - BufferValue2;
        }
        else
        {
            Icu_aActivePulseWidth[Channel] = \
                        (ICU_CAPTURE_REGISTER_MASK - BufferValue2) + BufferValue3;
        }
    }
    ...
    {
        Icu_aPeriod[Channel] = \
              (ICU_CAPTURE_REGISTER_MASK - BufferValue1) + BufferValue3;
    }
}
...
FUNC(void,ICU_CODE) Icu_Timestamp(VAR(Icu_ChannelType, AUTOMATIC)hwChannel,
                                  ...
                                  P2CONST(Icu_ValueType, ICU_VAR, ICU_APPL_DATA) pBufferPtr,
                                  VAR(boolean, AUTOMATIC) bOverflow
                                 )
{
    VAR(Icu_ChannelType, AUTOMATIC) Channel = (*(Icu_pCfgPtr->Icu_HwMap))[hwChannel];
    ...
    {
        CONST(Icu_IndexType, AUTOMATIC) chBufferSize   = Icu_aBufferSize[Channel] ;
        CONST(Icu_IndexType, AUTOMATIC) chBufferNotify = Icu_aBufferNotify[Channel] ;
...
{
    VAR(Icu_ChannelType, AUTOMATIC) Channel = (*(Icu_pCfgPtr->Icu_HwMap))[hwChannel];
    CONST(Icu_ChannelType, AUTOMATIC) msChannel = Channel;
...
FUNC(Icu_ValueType, ICU_CODE) Icu_GetCaptureRegisterValue(VAR(Icu_ChannelType, AUTOMATIC) Channel)
{
    ...
    VAR(Std_ReturnType,AUTOMATIC) validchannel1 = Icu_ValidateCallAndChannel \
                (Channel,(uint8)ICU_MODE_SIGNAL_MEASUREMENT, ICU_GET_CAPTURE_REGISTER_VALUE_ID);

    VAR(Std_ReturnType,AUTOMATIC) validchannel2 = Icu_ValidateCallAndChannel \
                (Channel,(uint8)ICU_MODE_TIMESTAMP, ICU_GET_CAPTURE_REGISTER_VALUE_ID);

    if (((Std_ReturnType)E_OK == validchannel1) || ((Std_ReturnType)E_OK == validchannel2))
    {
#endif
        tempValue = Icu_IPW_GetCaptureRegisterValue((*(Icu_pCfgPtr->Icu_ChannelId))[Channel]);
#if (ICU_VALIDATE_CALL_AND_CHANNEL == STD_ON)
    }
#endif
    return tempValue;
